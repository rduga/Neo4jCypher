...
public class check_Relationship_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode relationship, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    if (SNodeOperations.getAncestor(relationship, "neo4j.cypher.structure.CreateStatement", false, false) != null && ListSequence.fromList(SLinkOperations.getTargets(relationship, "type", true)).count() > 1) {
          ...
          if (SNodeOperations.getIndexInParent(it) > 0) {
            {
              ...
              IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(it, "Relationship can have only one type", "r:1549d4d9-195d-4192-a8ca-9bdca0139ffa(neo4j.cypher.typesystem)", "3682822877829156793", null, errorTarget);
            }
          }
        }
      });
